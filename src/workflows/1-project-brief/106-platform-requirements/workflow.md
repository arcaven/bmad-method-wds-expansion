...
```
Location: {output_folder}/_progress/agent-dialogs/{date}-platform-requirements.md
...
Load, read full file and then execute `{installed_path}/steps-c/step-01-init.md` to begin workflow.
...
- **Phase 6 (Design Deliveries)** — Development handoff specifications

---

## PRD RELATIONSHIP

The PRD (Product Requirements Document) lives in the **BMM module**, not WDS. WDS artifacts are referenced by the PRD — not duplicated into it.

**Two WDS injection points into the PRD:**
1. **Platform Requirements** (this workflow) — PRD references `platform-requirements.md` for technical boundaries, constraints, and integration decisions
2. **Design Deliveries** (Phase 6) — PRD references scenario specs, page specs, and delivery packages for the design contract

The BMM PM agent reads from these WDS artifacts when building the development plan. WDS owns the content, the PRD points to it.
